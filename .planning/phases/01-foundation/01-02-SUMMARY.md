...
provides:
  ...
  - IsProtected function in internal/fs: O(1) map lookup for 11 protected path names
  - ProtectedPaths function in internal/fs: sorted slice of all protected names for display/debug
  - testdata/python_shared_paths.txt: drift-detection fixture for SHARED_PATHS
...
**POSIX-atomic symlink create/replace via renameio/v2 and O(1) protected-path lookup with SHARED_PATHS drift detection**
...
- TestIsProtected_MatchesPythonConstants performs bidirectional fixture comparison to prevent silent drift
...
- `internal/fs/testdata/python_shared_paths.txt` — 11-line fixture of SHARED_PATHS
...
- Drift-detection test uses bidirectional comparison: entries in fixture but not Go map, and entries in Go map but not fixture, are both failures. This prevents the protected-paths set from silently diverging.
